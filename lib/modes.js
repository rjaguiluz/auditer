...
async function runTrivyMode(useExact, deps, devDeps) {
    ...
    if (answer === 'y' || answer === 'yes' || answer === '') {
      ...
      for (const [scope, data] of scopeFamiliesToUpdate) {
        ...
        if (prodPackagesWithVersions.length > 0) {
          ...
          if (run(`npm install${useExact ? ' --save-exact' : ''} ${prodPackagesWithVersions.join(' ')}`)) {
        ...
        if (devPackagesWithVersions.length > 0) {
          ...
          if (run(`npm install --save-dev${useExact ? ' --save-exact' : ''} ${devPackagesWithVersions.join(' ')}`)) {
  ...
  if (toUninstallProd.length || toUninstallDev.length) {
    ...
    if (prodWithVersions.length) {
      ...
      if (run(`npm install${useExact ? ' --save-exact' : ''} ${prodWithVersions.join(' ')}`)) {
        CHANGES_TRACKER.directUpdates.push(...trackProd);
      }
    }
    if (devWithVersions.length) {
      ...
      if (run(`npm install --save-dev${useExact ? ' --save-exact' : ''} ${devWithVersions.join(' ')}`)) {
        CHANGES_TRACKER.directUpdates.push(...trackDev);
      }
    }
  } else {
    runAuditFix();
