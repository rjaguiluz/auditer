...
function chooseClosestVersion(currentVersion, fixVersions) {
  ...
  // If no current version, return the lowest version available
  if (!currentVersion) {
    const sorted = [...fixVersions].sort(compareVersions);
    return sorted[0];
  }
  ...
  // First, try to find versions >= current
  ...
  // If no version >= current, take the highest available (edge case)
  if (!bestVersion) {
    const sorted = [...fixVersions].sort(compareVersions);
    bestVersion = sorted[sorted.length - 1];
  }
...
function findLatestMinorVersion(packageName, currentVersion) {
  try {
    ...
    const sorted = [...compatibleVersions].sort(compareVersions);
